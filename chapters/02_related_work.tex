...
\begin{itemize}
    ...
    \item \textbf{Semantic highlighting}, in contrast, uses the (externally linked) semantics referenced in the document to differentiate terms based on their roles. For instance, the type of an object or from a literal could be highlighted in a unique color, or keywords like JSON-LD's \texttt{@id} or \texttt{@graph}~\unsure{Isn't this more like sytactic highlighting?} can be visually distinguished from other keys.~\info{Same as previous comment: description/mention of the tools supporting this.}
\end{itemize}
