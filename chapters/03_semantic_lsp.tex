...
Note that each semantic language does its own tokenization, not allowing for SPARQL tokens inside Turtle documents.
...
With the language server protocol\todo{just use an acronym at all time here?} the server can specify wether or not it requires only the (minimal) changes or entire document every time.
...
An example is shown in \ref{lst:GroupedListing}.
  ...
  The user at \ref{code3} just wrote a subject, invoking the subject completion defined in the next section. The language server might incorrect suggest to add a comma after \texttt{<c>} to build a syntactically correct document.
  ...
  Adding partial parsing eliminates these issues as the language server understand which tokens are new and leaving the original interpretation of the surrounding tokens in touch. \todo{PC: please check this last sentence and rewrite it}
...
In the Langauge Server Protocol, the langauge server is issued a completion event that contains the current location of the user.
