...
If a system expects a component that is not present for a given entity, the system simply skips that entity.~\info{Can you add an example of this for clarity?}
...
Note that each Semantic Web language does its own tokenization, not allowing for SPARQL tokens inside Turtle documents.~\unsure{Not sure if this is good or bad. Can you rephrase to clarify your point?}
...
The next system is \textit{parsing}\info{Can we adjust Figure 1 so that the System is called parsing and differentiate from the Schedule called Parse?}, also implemented for each semantic language, transforming the tokens into actual objects and reporting on syntactic errors. 
...
Deriving triples does not only derive actual RDF triples, but in the case of SPARQL, also bindings~\info{By bindings you mean triple patterns? If so better say triple patterns. Bindings sounds more like the results of executing a query}, which can be thought of as \textit{triples}.

After deriving prefixes, other systems~\info{Which ones?} will create a \texttt{LinkedDocuments} component, listing all documents that are linked in some way to this document.
...
%\paragraph{About parsing} → you already have a heading parse?

The language server protocol\todo{just use an acronym at all time here?} allows the server to specify wether it requires only the (minimal) changes or the entire document every time.
...
An example is shown in Figure~\ref{lst:GroupedListing}.
  ...
  (iii) The user at figure \ref{code3} just wrote subject \texttt{<c>}, invoking the subject completion defined in the next section. The language server might incorrectly suggest to add a comma after \texttt{<c>} to build a syntactically correct document.~\info{Maybe explain what should be the correct outcome?}
  ...
  Adding partial parsing can deal with these issues as the language server understand which tokens are new and leaving the original interpretation of the surrounding tokens untouched. \todo{PC: please check this last sentence and rewrite it. Update by JR: I adjusted it a bit based on my interpretation, but do check if rephrasing is needed.}
